<script lang="ts">
	...
	let { messages, setMessages, infoCards = [] }: Props = $props();
	...
	function handleEdit(newText: string, message: Message) {
		const updatedMessages = $messages.map((m: Message) => {
			if (m.id === message.id) {
				return { ...m, content: newText.trim() };
			}
			return m;
		});
			...
			{#each $messages as message}
				...
				{:else if message.role === 'assistant'}
					<div use:scrollTo class="flex items-start bg-white py-4 px-3 w-full space-x-2 last:mb-14">
						...
						<EditableText text={message.content} on:edit={(e) => handleEdit(e.detail, message)} />
					</div>
				{/if}
			{/each}
		</div>
